...
void UpdateScale() 
{
	...
	float max = (fWidth > fHeight) ? fWidth : fHeight;
	float fSizeObject = max;
	float min = (m_nViewport[2] > m_nViewport[3]) ? m_nViewport[3] : m_nViewport[2];
	float fSizeView = min;
...
void ValidateConstraints()
{
	...
	getVertex(m_vSelected, m_deformedMesh,m_deformer);
	while ( cur != end ) {
...
{
	if ( key == 'f' ) {
		CFileDialog openDlg(TRUE, _T("obj"), _T("mesh.obj"), 0, _T("OBJ Files (*.obj)|*.obj|"));
		if ( openDlg.DoModal() == IDOK ) {

			m_mesh.Clear();
			CStringA charstr((LPCTSTR)openDlg.GetPathName());
			m_mesh.read((const char*)charstr);
			m_vSelected.clear();
			InitializeDeformedMesh();
