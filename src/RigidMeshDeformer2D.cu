...
void RigidMeshDeformer2D::InitializeFromMesh( TriangleMesh * pMesh )
{
	...
	for ( unsigned int i = 0; i < nTris; ++i ) {
		...
		Triangle_GPU & t_c = m_vTriagles_CPU[i];
	...
	memset(Matrix_CPU, 0.0, 2 * 2 * nVerts*nVerts*sizeof(double));
	cudaMalloc((void**)&m_vVertexMap_GPU, sizeof(int)*nVerts);
}
...
void RigidMeshDeformer2D::UpdateConstraint( Constraint & cons )
{
	...
	if ( found != m_vConstraints.end() ) {
		...
		const_cast<Wml::Vector2f&>((*found).vConstrainedPos) = const_cast<Wml::Vector2f&>(cons.vConstrainedPos);
...
void RigidMeshDeformer2D::PrecomputeOrientationMatrix()
{
	...
	//cudaMalloc((void**)&m_vVertexMap_GPU, sizeof(int)*nVerts);
	cudaMemcpy(m_vVertexMap_GPU, m_vVertexMap.data(), sizeof(int)*nVerts, cudaMemcpyHostToDevice);
	// ok, now fill matrix (?)
	size_t nTriangles = m_vTriangles.size();
...
//		_RMSInfo("Triangle %d: \n", i);
		...
		for ( int j = 0; j < 3; ++j ) {
			...
			if ((i==0||i==1)&&j==0)
				printf("test CPU value %d,%d n0x %d n0y %d x %d y %d\n", i,j,n0x, n0y, x, y);
